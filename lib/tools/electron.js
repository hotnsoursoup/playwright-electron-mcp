...
export default function electronTools() {
    return [
        defineTool({
            capability: 'core',
            schema: {
        ...
        defineTool({
            ...
            handle: async (context, args) => {
                ...
                const windowInfo = await Promise.all(windows.map(async (window, index) => ({
                    index,
...
function getElectronApp(context) {
    ...
    const electronApp = context._electronApp || context._browser?._electronApp;
    if (!electronApp) {
