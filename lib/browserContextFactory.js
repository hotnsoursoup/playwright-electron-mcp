...
class ElectronContextFactory {
    ...
    async createContext() {
        ...
        const launchOptions = {
            ...
            cwd: this.browserConfig.launchOptions?.cwd,
            env: this.browserConfig.launchOptions?.env,
        };
        this._electronApp = await _electron.launch(launchOptions).catch((error) => {
            if (error.message.includes('Executable doesn\'t exist'))
        ...
        browserContext._electronApp = this._electronApp;
        const close = async () => {
